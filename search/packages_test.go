...
func TestSearchPackages(t *testing.T) {
	...
	references, err := SearchPackages(&servicesManager, "repository", "name/version", false)
	assert.Nil(t, err)
	assert.Equal(t, 8, len(references))
}

func TestSearchPackagesLatest(t *testing.T) {
	servicesManager := MockRtServicesManagerPackages{}
	params := services.NewSearchParams()
	params.Pattern = "the/pattern/to/search/for"
	params.Recursive = false
	...
	references, err := SearchPackages(&servicesManager, "repository", "name/version", true)
	assert.Nil(t, err)
